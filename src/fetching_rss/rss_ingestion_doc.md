...
## 1. `read_feeds.rs` (O Trator de Ingestão HTTP)
...
### `read_feed`
- **Motivo de criação:** O *Worker* individual. Acessa uma URI estrita, executa a request `GET`, aguarda o corpo do stream HTTP, aciona o interpretador `feed_rs`, retalha as `Entry` XML nativas pro nosso formato proprietário de Struct `NewsItem` descartando a imensa gordura de campos nativos dos RSS (guid, autores corporativos, meta-tags que não nos interessam).
- **Quem chama:** Módulo interno pela função `fetch_rss_news`.
- **Motivo da chamada:** Extração de sub-elementos pontuais e sanitização de um jornal por vez.
- **Necessidade (0 a 10):** 10.

---

## 2. `feed_days.rs` (Janela de Residente Temporal)
...
- **Quem chama:** Módulo interno `read_feed`.
