# Documentação Técnica: Motor de Curadoria (`src/curation`)
...
- **Quem chama:** `curate_news_for_llm`.
...
## 2. `priority.rs` (Atribuidor de Peso)
...
## 3. `signals.rs` (Censores Semânticos)

### `is_technical_or_security`
...
- **Quem chama:** `collect_ranked_news` e `source_cap`.
...
## 4. `sources.rs` (Radares de Domínios)

### `is_high_volume_general_source`
...
- **Quem chama:** `is_technical_or_security` e `source_cap`.
